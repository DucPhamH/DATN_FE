...
export default function ModalUploadCoverAvatar({ closeModalCoverAvatar }) {
  ...
  return (
    <ModalLayout
      closeModal={closeModalCoverAvatar}
      className='modal-content max-h-[90%] min-w-[360px] md:min-w-[450px] dark:bg-gray-900 bg-[#f7f1db]'
    >
      <div className='relative w-full max-w-md max-h-full'>
        <div className='px-2 text-center '>
          <h3 className='mb-6 font-medium text-lg md:text-xl text-black dark:text-gray-200'>Chọn ảnh nền</h3>
            ...
            <div className='max-w-sm mx-auto dark:bg-slate-950 bg-white rounded-lg shadow-md overflow-hidden items-center'>
              <div className='px-4  py-6'>
                <div className='flex justify-center items-center' onClick={handleImageClick}>
                  {image ? (
                    <img
                      className='h-[16rem] w-[18rem] border border-red-600 mb-5  object-contain'
                      src={URL.createObjectURL(image)}
                      alt='avatar'
                    />
                  ) : (
                    <div className='max-w-sm h-[16rem] w-[18rem] flex justify-center mb-4 dark:bg-slate-950  bg-gray-100 border-dashed border-2 border-gray-400  items-center mx-auto text-center cursor-pointer'>
